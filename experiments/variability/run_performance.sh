...
DATA_DIR="$BASE_DIR/data/our_models_ctm"

# HLDA/HDP Configuration
SCRIPT_DIR="$BASE_DIR/experiments/variability"
PYTHON_SCRIPT="$SCRIPT_DIR/train_htms_performance.py"
VENV="$BASE_DIR/.venv_kumo01/bin/activate"
#MODELS=("htm-ds") # "htm-ws" 
...
run_task() {
    ...
    #local path_corpus="/export/usuarios_ml4ds/lbartolome/Datasets/$corpus_name/models_preproc/iter_0/corpus.txt"
    local path_corpus="/export/usuarios_ml4ds/lbartolome/Datasets/$corpus_name/models_preproc_ctm/iter_0/corpus.parquet"

    # Prepare the Python command
    local cmd="python \"$python_script\" --path_corpus \"$path_corpus\" --version \"$model_type\" --start \"$iteration\" --ntopics_root \"${N_TOPICS[$corpus_name]}\" --models_folder \"$model_dir\""
