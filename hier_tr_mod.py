...
class HierarchicalTrainer:
    ...
    def train(self):
        ...
        for epoch in tqdm(range(1, self.epochs + 1), leave=False):
            ...
            for batch_data in self.dataset.train_dataloader:
                input_bow = batch_data["train_bow"].to("cuda")  # Move to the correct device
                input_bow = input_bow.squeeze(1)
                #print("IT is here")
                #print("Batch data shape:", batch_data["train_bow"].shape)
                #print("Input to model shape:", input_bow.shape)
                
                rst_dict = self.model(input_bow)
                
                batch_loss = rst_dict['loss'].mean()
            ...
            if epoch % self.log_interval == 0:
                ...
                for key in loss_rst_dict:
                    output_log += f' {key}: {(loss_rst_dict[key] / data_size).item():.3f}'
    ...
    def test(self, bow):
        ...
        with torch.no_grad():
            self.model.eval()
            for idx in all_idx:
                batch_input = torch.FloatTensor(bow[idx].todense()).to("cuda")

                #batch_input = torch.FloatTensor(self.bow[idx].todense()) #bow[idx]
